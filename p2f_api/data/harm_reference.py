...
class harm_reference(baseSQL):
    ...
    reference_id: Mapped[UUID] = mapped_column(Uuid, unique=True, nullable=False, default=func.gen_random_uuid())
...
class harm_reference_to_record(baseSQL):
    ...
    pk_harm_reference_to_record: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    fk_harm_reference: Mapped[UUID] = mapped_column(ForeignKey("p2f_harm_reference.reference_id"))
