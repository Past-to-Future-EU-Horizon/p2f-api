...
def list_harm_data_types(
    ...
    logger.debug(
        f"🕸️📃 {__name__} {stack()[0][3]}({measure}, {unit_of_measure}, {method}, {dataset_id})"
    )
...
def get_harm_data_type(datatype_id: uuid.UUID) -> Harm_data_type:
    logger.debug(f"🕸️🔎 {__name__} {stack()[0][3]}({datatype_id})")
    return harm_data_types.get_harm_data_type(datatype_id=datatype_id)


@router.post("/")
def insert_harm_data_type(new_harm_data_type: Harm_data_type) -> Harm_data_type:
...
def delete_harm_data_record(datatype_id: uuid.UUID) -> None:
