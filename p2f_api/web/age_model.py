...
router = APIRouter(prefix="/age-models", tags=[tag_name])

tag_metadata = {"name": tag_name,
                "description": 
                """The age model is metadata about how the age of a data point was calculated for a dataset or an individual record.  
The age model is descriptive, but should be re-usable across the entire project. 

The age model has three schema parts: 

* age_model_id : A unique identifier for the age model, this will be generated by the API server, do not generate yourself. 
* age_model_name : An easy to read name for an age model that should be standard within a community.
* age_model_description : Any extra information about how an age model was created, or a link to a paper describing its standard."""}
...
# Delete
@router.delete("/{age_model_id}", operation_id="agemodel-delete", include_in_schema=False)
