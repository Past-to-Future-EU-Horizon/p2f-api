...
router = APIRouter(prefix="/keywords", tags=[tag_name])

tag_metadata = {"name": tag_name,
                "description": 
                """Keywords are words or phrases that can be associated with a dataset. 
You can add keywords freely to a dataset using the keyword endpoints here. 
This API will also be loaded with dictionaries of taxonomies from various standaridization projects, and these can then be associated with a dataset. 

To control the dictionaries, this will be loaded from the backend of the API, you can create any keyword you like on the frontend. 
You can associate taxons from dictionaries using the assign endpoint. """}
...
def remove_taxon_from_dataset(auth: api_token_annotation,
                             taxdict_id: str, 
                             dataset_id: uuid.UUID):
    return keywords.remove_taxon_from_dataset(taxdict_id=taxdict_id, 
                                             dataset_id=dataset_id)