...
tag_name = "HARM Reference"

router = APIRouter(prefix="/harm-reference", tags=[tag_name])

tag_metadata = {"name": tag_name,
                "description": 
                """HARM References are a way to standardize referencing other datasets and publications to enable use to cite the scientists that have contributed so much to future work. 

This metadata node is subject to change.

HARM References have the following attributes:

* reference_id : A unique identifier in the UUID format for a reference and generated by the API, do not generate yourself
* doi : A digital object identifier to refer to an existing dataset or piece of literature
* other_link : Any other HTTPS URL for a dataset or piece of literature. 
* reference_type : A description of the resource type, such as article, book, dataset, etc. 
* reference_content : Description or extra information of the reference. """}
