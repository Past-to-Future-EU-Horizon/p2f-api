...
def get_harm_data_record() -> Harm_data_record:
...
def delete_harm_data_record(existing_pk: int) -> None:
    logger.debug("🗑️ service/harm_data_record.py create_dataset()")
    with Session(engine) as session:
        logger.debug("\tCreated session")
        stmt = delete(harm_data_record).where(harm_data_record.pk_harm_data_record == existing_pk)
        execute = session.execute(stmt)
        commit = session.commit()
    return None